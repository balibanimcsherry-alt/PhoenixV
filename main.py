...
if _dist.exists():
    ...
    async def _redirect_availability():
        return _RedirectResponse('/book', status_code=301)
    ...
    async def _spa(full_path: str):
        ...
        # Serve prerendered static HTML if it exists (generated by prerender.mjs)
        prerendered = _dist / full_path / 'index.html'
        if prerendered.exists() and prerendered.is_file():
            return _FileResponse(str(prerendered), headers={'Cache-Control': 'no-cache, no-store, must-revalidate'})
